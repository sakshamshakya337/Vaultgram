          ...
          <div className="flex items-center gap-1.5 shrink-0 ml-2">
            ...
            {errorFiles > 0 && (
              <button
                ...
                className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/30 text-xs font-semibold transition-all cursor-pointer shadow-sm"
                title={`Retry ${errorFiles} failed ${errorFiles === 1 ? 'upload' : 'uploads'}`}
                aria-label={`Retry ${errorFiles} failed uploads`}
              >
                <RotateCw className="w-3.5 h-3.5" />
                <span className="text-[11px] font-bold">Retry {errorFiles > 1 ? `(${errorFiles})` : 'All'}</span>
              </button>
            )}
                    ...
                    <div className="flex items-center gap-2.5 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {isError && item.errorMessage && (
                          <p className="text-[10px] text-rose-400 font-medium truncate mt-0.5" title={item.errorMessage}>
                            {item.errorMessage}
                          </p>
                        )}
            ...
            <div className="flex items-center gap-2">
              ...
              {queuedFiles === 0 && !activeItem && (
                <button
                  ...
                  className="text-[11px] font-semibold text-zinc-500 hover:text-rose-300 transition-colors cursor-pointer"
                >
                  Clear all
                </button>
              )}
